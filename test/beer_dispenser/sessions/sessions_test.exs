defmodule BeerDispenser.SessionsTest do
  use BeerDispenser.DataCase

  alias BeerDispenser.{Accounts, Sessions, Sessions.Session}
  ...
  describe "write session data" do
    ...
    test "create_session/1 with custom max_age / expiry time", %{user: user} do
      create_attrs = %{user_id: user.id, max_age: 7_200}
      assert {:ok, %Session{} = session} = Sessions.create_session(create_attrs)
      assert session.user_id == user.id
      assert DateTime.diff(session.expires_at, DateTime.utc_now()) == 7_200
    end
  end
