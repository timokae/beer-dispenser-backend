defmodule BeerDispenserWeb.Email do
  @moduledoc """
  ...
  alias BeerDispenser.Accounts.User
  alias BeerDispenserWeb.Mailer
  ...
  def send_activation_email(%User{} = user) do
    Mailer.activation_email(user)
    |> Mailer.deliver_now()

    Logger.info("Sent activation email to #{user.email}")
  end

  def send_invoice_email(%User{} = user, amount) do
    Mailer.invoice_email(user, amount)
    |> Mailer.deliver_later()

    Logger.info("Sent invoice email email to #{user.email}")
  end

  def send_paypal_confirmation_email(%User{} = user) do
    Mailer.paypal_confirmation_email(user)
    |> Mailer.deliver_later()
  end
end
