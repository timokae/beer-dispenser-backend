defmodule BeerDispenserWeb.Authorize do
  @moduledoc """
  ...
  def admin_check(%Plug.Conn{assigns: %{current_user: current_user}} = conn, _opts) do
    if User.has_role?(current_user, "admin") do
      conn
    else
      conn
      |> put_flash(:error, "You are not authorized to view this page")
  ...
  def super_admin_check(%Plug.Conn{assigns: %{current_user: current_user}} = conn, _opts) do
    if User.has_role?(current_user, "superadmin") do
      conn
    else
      conn
      |> put_flash(:error, "You are not authorized to view this page")
