defmodule BeerDispenserWeb.SessionController do
  use BeerDispenserWeb, :controller

  import BeerDispenserWeb.Authorize

  alias Phauxth.Remember
  alias BeerDispenser.{Sessions, Sessions.Session}
  alias BeerDispenserWeb.Auth.Login
  ...
  def create(conn, %{"session" => params}) do
    case Login.verify(params) do
      {:ok, user} ->
        ...
        |> add_remember_me(user.id, %{"remember_me" => "true"})
  ...
  defp add_remember_me(conn, user_id, %{"remember_me" => "true"}) do
    Remember.add_rem_cookie(conn, user_id, @max_age)
  end
